...
class HCRing:                       # Defines a molecule of linear aromatic rings
    def __init__(self, n):
...
def listEnergies(molecule):         # Accesses an instance of HCRing and does PiaB modelling of its energy states
    E0 = (con.hbar**2 * con.pi**2)/(2*con.m_e)
    earray = []
    nxarray = []
    nyarray = []
    for nx in range(1,molecule.lumo + 3):    #To capture all energy levels, as nx increases energy by small amounts,
        for ny in range(1,4):                #While ny increases energy by much larger amounts.
            energy = (E0 * ((nx**2 / (molecule.lx)**2) + (ny**2 / (molecule.ly)**2)))/con.e
            earray.append([energy])
            nxarray.append([nx])
            nyarray.append([ny])
    a = np.hstack([nxarray,nyarray,earray])
    sort = a[a[:,2].argsort()]               #Stacking and sorting syntax took a while to get the desired outpit
    return sort
